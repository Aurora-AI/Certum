...
const Hero: React.FC = () => {
  ...
  const scrambledLine1 = useScrambleText('Wealth', { duration: 1.5, delay: 0.5, speed: 40 });
  ...
  useEffect(() => {
    ...
    if (blendTextRef.current) gsap.set(blendTextRef.current, { opacity: 0, color: '#ffffff' });
    if (tipRef.current) gsap.set(tipRef.current, { opacity: 0, color: '#ffffff' });

    if (heroBg) {
      tl.fromTo(heroBg, { scale: 1.08 }, { scale: 1, duration: reduceMotion ? 0 : 2.2, ease: 'expo.out' }, 0);
    }

    tl.to(blendTextRef.current, { opacity: 1, duration: reduceMotion ? 0 : 0.8, delay: 0.5, ease: 'power2.out' }, 0)
      .to(tipRef.current, { opacity: 1, duration: reduceMotion ? 0 : 0.8, delay: 0.7, ease: 'power2.out' }, 0)
      .add('order', 1.0)
      .to(blendBgRef.current, { opacity: 1, duration: reduceMotion ? 0 : 2.5 }, 'order')
      .to(blendTextRef.current, { color: '#000000', duration: reduceMotion ? 0 : 2.5 }, 'order')
      .to(tipRef.current, { color: '#050505', duration: reduceMotion ? 0 : 2.5 }, 'order')
      .fromTo(
        contentRef.current,
        { y: '5vh', opacity: 0 },
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-10 left-10 z-30 transition-opacity duration-700 group-hover:opacity-0 pointer-events-none">
        <span ref={tipRef} className="font-mono text-[10px] tracking-[0.25em] uppercase">
          Vislumbrar Futuro
        </span>
      </div>
      ...
      >
        <div className="col-span-12 md:col-span-8 flex flex-col justify-between min-h-screen py-12 md:py-24 pr-0 md:pr-12">
          <div className="flex items-center gap-3 opacity-60 pointer-events-auto mix-blend-multiply">
            ...
            <span className="font-mono text-xs uppercase tracking-widest text-ink">Protocol Genesis v.3</span>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 pointer-events-auto items-end">
            <div className="max-w-md">
              <p className="text-lg md:text-xl text-ink leading-relaxed font-medium">
                O sistema operacional para alavancagem soberana. Acesso a capital institucional sem fricção
                bancária.
              </p>
            </div>
            <div className="flex gap-12 border-l-2 border-ink pl-8 py-2">
              <div>
                <p className="font-mono text-[10px] text-muted uppercase tracking-widest mb-1">TVL</p>
                <p className="text-4xl text-ink font-bold tracking-tight">R$ 14.2Bi</p>
              </div>
              <div>
                <p className="font-mono text-[10px] text-muted uppercase tracking-widest mb-1">Yield</p>
                <p className="text-4xl text-emerald-600 font-bold tracking-tight">+18.4%</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-12 md:col-span-4 flex flex-col justify-between min-h-screen py-12 md:py-24 pointer-events-auto pl-0 md:pl-12">
          ...
          <div className="flex flex-col gap-6 items-start md:items-end pb-2 md:pb-0">
            <button className="group relative w-full md:w-auto px-10 py-6 bg-ink text-white text-xs font-bold uppercase tracking-[0.2em] overflow-hidden transition-all hover:bg-neutral-900">
              <span className="relative z-10 flex items-center justify-center gap-3">Initiate Protocol</span>
